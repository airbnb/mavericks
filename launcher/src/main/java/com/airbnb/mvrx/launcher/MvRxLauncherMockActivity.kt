...
class MvRxLauncherMockActivity : MvRxBaseLauncherActivity() {
        ...
        if (savedInstanceState == null) {
            showNextMock(
                ...
                onFailure = {
                    ...
                    toastLong("Fragment crashed - see logcat for stacktrace.")
                    finish()
                }
            )
        }
    ...
    companion object {
         ...
         */
        fun intent(context: Context, mock: MockedViewProvider<*>): Intent {
            Log.d(
                tag,
                "Creating mvrx launcher intent for ${activityToShowMock::class.java.simpleName} for view ${mock.viewName}"
            )
            nextMockToShow = mock
            return Intent(context, activityToShowMock.java)
        }
         ...
         * The implementation will depend on the type of [MvRxView].
         ...
         */
        fun mockBehavior(mock: MvRxMock<out MvRxView, out Parcelable>): MockBehavior {
            return when {
