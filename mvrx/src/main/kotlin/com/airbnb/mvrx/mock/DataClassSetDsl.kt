...
interface DataClassSetDsl {
    ...
    fun <T, Type> Async<T>.success(block: T.() -> KProperty0<Type>): KProperty0<Type> {
        val successValue = this.invoke()
            ?: throw IllegalStateException("Async value is not in the success state, it is `${this::class.simpleName}`")
    ...
    class Setter<DataClass : Any, PropType>(val instance: DataClass, val property: KProperty0<PropType>) {
        init {
            ...
            if (instance is Async<*>) {
                require(instance is Success<*>) { "Cannot set property ${property.name}, the Async value it is in is not in the Success state" }
            } else {
                require(clazz.isData) { "${clazz.simpleName} must be a data class to change mock values with 'set'" }
            }
        ...
        fun getValue(dataClass: DataClass): PropType {
            ...
            return propertyChain.fold<KProperty0<Any?>, Any>(dataClass) { data, kProp0 ->
                val kProp1 = data::class.memberProperties.singleOrNull { it.name == kProp0.name }
                    ?: throw IllegalStateException("Could not find property of name ${kProp0.name} on class ${data::class.simpleName}")

                kProp1.call(data) ?: throw IllegalStateException("kProp1.call(recursiveDataClass) as PropType")
            } as PropType
        }
        ...
        fun set(value: PropType): DataClass {
            ...
            return if (instance is Success<*>) {
                val successValue = instance.invoke()
                    ?: throw IllegalStateException("Success value is null - cannot set ${property.name}")
