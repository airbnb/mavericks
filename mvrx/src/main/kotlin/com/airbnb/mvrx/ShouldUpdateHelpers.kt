 ...
 *
 * @param initialValue determines whether or not the subscriber will be called with the initial state.
 */
fun <S : Any> propertyWhitelist(
        prop1: KProperty1<S, *>,
        initialValue: Boolean = true): (S?, S
) -> Boolean = { oldState, newState ->
    if (oldState == null && !initialValue) false
 ...
 *                     if it is already [Success].
 */
fun <S : Any, T : Async<*>> onSuccess(property: KProperty1<S, T>, initialValue: Boolean = true): (S?, S) -> Boolean = { oldState, newState ->
    if (oldState == null && !initialValue) false
 ...
 *                     if it is already [Success].
 */
fun <S : Any, T : Async<*>> onFail(property: KProperty1<S, T>, initialValue: Boolean = true): (S?, S) -> Boolean = { oldState, newState ->
    if (oldState == null && !initialValue) false
